...
Não há testes automatizados ou linting; `verificar_solucao.py` (raiz do repo) é o teste de regressão manual — roda o pipeline completo com a planilha ativa e audita a solução (limites, não-sobreposição, apoio mínimo de `APOIO_MIN_PCT`%, hoje 60%).
